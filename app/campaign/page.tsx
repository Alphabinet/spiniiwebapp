...
const SignInPrompt = () => {
    ...
    return (
        <div className="flex flex-col items-center justify-center py-20 text-center bg-white rounded-xl shadow-md border">
            <Lock className="w-16 h-16 mx-auto text-violet-400" />
            <h3 className="mt-6 text-2xl font-bold text-gray-800">Please Sign In</h3>
            <p className="mt-2 text-gray-500 max-w-sm">
                Log in or create an account to discover and apply for exclusive campaigns from top brands.
            </p>
...
export default function CampaignPage() {
    ...
    const proceedWithApplication = async (campaignId: string, igUsername: string, igLink: string) => {
        ...
        const applicantData = {
            ...
            instagramProfile: igLink,
        };

        await setDoc(doc(db, `campaigns/${campaignId}/applicants`, user.uid), applicantData);
        await setDoc(doc(db, `users/${user.uid}/appliedCampaigns`, campaignId), { appliedAt: new Date().toISOString(), campaignName: allCampaigns.find(c => c.id === campaignId)?.name || 'Campaign' });

        onApplicationChange();
        if (selectedCampaign) setSelectedCampaign(null);
    };

    const initiateApplication = (campaignId: string) => {
        ...
        if (userProfile.instagramUsername && userProfile.instagramProfile) {
            ...
            setInstagramUsername(userProfile.instagramUsername || '');
            setInstagramLink(userProfile.instagramProfile || '');
            setIsInstagramModalOpen(true);
        }
    };

    const handleDirectApply = async (campaignId: string) => {
        setIsApplying(true);
        try {
            await proceedWithApplication(campaignId, userProfile!.instagramUsername!, userProfile!.instagramProfile!);
    ...
    const handleSaveInstagramAndApply = async () => {
        if (!instagramUsername.trim() || !instagramLink.trim()) {
            toast.error("Please fill in both Instagram username and profile link.");
            return;
        }
        if (!user || !applyingToCampaignId) return;
        ...
        try {
            ...
            setUserProfile(prev => prev ? { ...prev, instagramUsername, instagramProfile: instagramLink } : null);
            await proceedWithApplication(applyingToCampaignId, instagramUsername, instagramLink);

            toast.success("Profile updated & application sent!");
            setIsInstagramModalOpen(false);
            setApplyingToCampaignId(null);
        } catch (error) {
            console.error("Error saving profile and applying:", error);
    ...
    const handleUnapply = async (campaignId: string) => {
        if (!user) return;
        setIsApplying(true);
        try {
                    ...
                    <>
                        ...
                        <div className="flex flex-col gap-4 p-4 mb-6 bg-white rounded-xl shadow-md sm:flex-row">
                            ...
                            <Select onValueChange={setPlatformFilter} value={platformFilter}>
                                ...
                                <SelectContent><SelectItem value="all">All Platforms</SelectItem><SelectItem value="Instagram">Instagram</SelectItem><SelectItem value="Youtube">YouTube</SelectItem><SelectItem value="Facebook">Facebook</SelectItem></SelectContent>
                            </Select>
                            <Select onValueChange={setSortOrder} value={sortOrder}>
                                <SelectTrigger className="w-full sm:w-48"><SelectValue placeholder="Sort by" /></SelectTrigger>
                                <SelectContent><SelectItem value="latest">Latest</SelectItem><SelectItem value="oldest">Oldest</SelectItem><SelectItem value="budget-high-low">Budget: High to Low</SelectItem><SelectItem value="budget-low-high">Budget: Low to High</SelectItem><SelectItem value="popularity">Most Popular</SelectItem></SelectContent>
                            </Select>
                        </div>

                        {isLoading ? (
                            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">{[...Array(6)].map((_, i) => <CampaignSkeleton key={i} />)}</div>
                        ) : filteredAndSortedCampaigns.length > 0 ? (
                            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                                {filteredAndSortedCampaigns.map((campaign) => (
                                    <CampaignCard key={campaign.id} campaign={campaign} user={user} userProfile={userProfile} isApplied={appliedCampaignIds.has(campaign.id)} onViewDetails={() => setSelectedCampaign(campaign)} onApply={() => initiateApplication(campaign.id)} onUnapply={() => handleUnapply(campaign.id)} isProcessing={isApplying} isExpanded={expandedCampaignId === campaign.id} onToggleExpand={() => toggleCampaignExpansion(campaign.id)} />
                                ))}
                            </div>
                        ) : (
                            <div className="flex flex-col items-center justify-center py-12 text-center bg-white rounded-xl shadow-sm">
                                <PartyPopper className="w-16 h-16 mx-auto text-violet-500" />
                                <h3 className="mt-4 text-xl font-semibold">No Approved Campaigns Found</h3>
                                <p className="text-gray-500">Check back later for new opportunities!</p>
                                <Button onClick={() => { setNameFilter(''); setPlatformFilter('all'); setSortOrder('latest'); }} className="mt-4">Reset Filters</Button>
                            </div>
                        )}
                    </>
                )}
                        ...
                        <div className="flex items-start justify-between pb-4 border-b">
                            <div>
                                <div className="flex items-center">
                                    {selectedCampaign.brandLogo ? <Image src={selectedCampaign.brandLogo} alt="Brand logo" width={48} height={48} className="mr-4 rounded-full" /> : <div className="flex items-center justify-center w-12 h-12 mr-4 bg-gray-100 rounded-full"><User className="w-6 h-6 text-gray-500" /></div>}
                                    <div>
                                        <h2 className="text-2xl font-bold">{selectedCampaign.name}</h2>
                                        <div className="flex items-center mt-1"><PlatformIcon platform={selectedCampaign.platform} /><span className="ml-2 text-gray-500">{selectedCampaign.platform} Campaign</span></div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="p-4 mt-6 bg-violet-50 rounded-xl">
                            ...
                            <div className="mt-4">
                                ...
                                <ul className="mt-2 space-y-1">
                                    {selectedCampaign.services.reel > 0 && <li className="flex items-center"><span className="mr-2">•</span> Reels: {selectedCampaign.services.reel}</li>}
                                    {selectedCampaign.services.story > 0 && <li className="flex items-center"><span className="mr-2">•</span> Stories: {selectedCampaign.services.story}</li>}
                                    {selectedCampaign.services.reelAndStory > 0 && <li className="flex items-center"><span className="mr-2">•</span> Reel + Story: {selectedCampaign.services.reelAndStory}</li>}
                                </ul>
                            </div>
                        </div>
                        <div className="mt-6">
            ...
            <Dialog open={isInstagramModalOpen} onOpenChange={setIsInstagramModalOpen}>
                <DialogContent className="sm:max-w-[425px]">
                    <DialogHeader>
                        ...
                        <DialogDescription>Please provide your Instagram username and profile link to apply. This is a one-time step.</DialogDescription>
                    </DialogHeader>
                    <div className="grid gap-4 py-4">
                        <div className="grid grid-cols-4 items-center gap-4">
                            <label htmlFor="ig-username" className="text-right">Username</label>
                            <Input id="ig-username" value={instagramUsername} onChange={(e) => setInstagramUsername(e.target.value)} className="col-span-3" placeholder="@username" />
                        </div>
                        <div className="grid grid-cols-4 items-center gap-4">
                            <label htmlFor="ig-link" className="text-right">Profile Link</label>
                            <Input id="ig-link" value={instagramLink} onChange={(e) => setInstagramLink(e.target.value)} className="col-span-3" placeholder="instagram.com/..." />
                        </div>
                    </div>
                    <DialogFooter>
                        <Button onClick={handleSaveInstagramAndApply} disabled={isSavingInstagram}>
                            {isSavingInstagram ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : null}
                            Save and Apply
                        </Button>
                    </DialogFooter>
                </DialogContent>
...
const CampaignCard: React.FC<CampaignCardProps> = ({ campaign, user, userProfile, isApplied, isProcessing, onViewDetails, onApply, onUnapply, isExpanded, onToggleExpand }) => {
    ...
    const getStatusClasses = (status: string) => {
        switch (status) {
            case 'Approved': case 'Ongoing': return 'bg-blue-100 text-blue-800';
            ...
            <div className="flex-grow">
                <div className="flex items-start justify-between mb-3">
                    <div>
                        <div className="flex items-center gap-2">
                            {campaign.brandLogo ? <Image src={campaign.brandLogo} alt="Brand logo" width={32} height={32} className="rounded-full" /> : <div className="flex items-center justify-center w-8 h-8 bg-gray-100 rounded-full"><User className="w-5 h-5 text-gray-500" /></div>}
                            <h3 className="pr-4 text-lg font-bold">{campaign.name}</h3>
                        </div>
                        <div className="flex items-center mt-1">
                            <Badge className={`${getPlatformColor(campaign.platform)} rounded-full`}><PlatformIcon platform={campaign.platform} /><span className="ml-1">{campaign.platform}</span></Badge>
                            <Badge className={`ml-2 ${getStatusClasses(campaign.status)} rounded-full`}>Ongoing</Badge>
                        </div>
                    </div>
                ...
                {isExpanded && (
                    <div className="pt-4 mt-4 space-y-3 border-t">
                        <div className="flex items-start"><MapPin className="flex-shrink-0 w-4 h-4 mt-0.5 mr-2 text-violet-500" /><p className="text-sm text-gray-600">{campaign.location}</p></div>
                        <div className="flex items-start"><Hash className="flex-shrink-0 w-4 h-4 mt-0.5 mr-2 text-violet-500" /><p className="text-sm text-gray-600">{campaign.creatorCategory}</p></div>
                        <div className="flex items-center"><Cake className="flex-shrink-0 w-4 h-4 mr-2 text-violet-500" /><p className="text-sm text-gray-600">{campaign.minAge}-{campaign.maxAge} years</p></div>
                    </div>
                )}
            </div>
            <div className="flex gap-2 pt-4 mt-auto border-t">
                ...
                {!user ? (
                    ...
                    <Button className="w-full text-white bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-700 hover:to-indigo-700" onClick={onApply} disabled={isProcessing}>
                        {isProcessing ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <CheckCircle className="w-4 h-4 mr-2" />}Apply
                    </Button>
                )}
            </div>
        </div>
